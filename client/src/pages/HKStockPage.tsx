...
export default function HKStockPage() {
  ...
  const loadWatchlist = useCallback(async () => {
    try {
      ...
      const hkList = data.filter((d) => d.type === 'hk_stock');
      setWatchlist(hkList);
      return data;
    } catch {
      message.error('加载自选港股失败');
      return [];
    }
  }, []);
  ...
  const loadInvestments = useCallback(async () => {
    try {
      ...
      setInvestments(data.filter((d) => d.type === 'hk_stock'));
    } catch {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const hkSymbols = data.filter((d) => d.type === 'hk_stock').map((d) => d.symbol);
      if (hkSymbols.length > 0) {
        socketService.onQuotes('hkStockPage', (newQuotes) => {
          setQuotes((prev) => {
        ...
        socketService.subscribe(hkSymbols);
      }
    };
    ...
    return () => {
      socketService.offQuotes('hkStockPage');
  ...
  const handleSearch = async (keyword: string) => {
    ...
    try {
      ...
      setSearchResults(Array.isArray(results) ? results.filter((r) => r.type === 'hk_stock') : []);
    } catch {
      message.error('搜索失败');
    } finally {
      setSearching(false);
  ...
  const handleAddStock = async (item: SearchResult) => {
    try {
      await watchlistApi.add(item.symbol, item.name, 'hk_stock');
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Space>
          <Title level={4} style={{ margin: 0 }}>
            <DollarOutlined /> 自选港股 ({watchlist.length})
          </Title>
      ...
      {filteredWatchlist.length === 0 ? (
        <Card style={{ textAlign: 'center', padding: 40 }}>
          ...
          <Title level={5} type="secondary">暂无自选港股</Title>
          <Text type="secondary">点击上方"添加港股"按钮搜索添加</Text>
        </Card>
      ) : (
        <div
          style={{
      ...
      >
        <Space.Compact style={{ width: '100%', marginBottom: 16 }}>
          <Input
            placeholder="输入港股代码/名称，如: 00700、腾讯"
            value={searchKeyword}
                ...
                title={
                  <span>
                    ...
                    <Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
                      {item.symbol.toUpperCase().replace('HK_', '')}
                    </Text>
                  </span>
                }
              />
      ...
      {/* K线图弹窗 */}
      <Modal
        title={`${chartName} (${chartSymbol?.toUpperCase().replace('HK_', '')})`}
        open={!!chartSymbol}
