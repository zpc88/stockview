...
import { PlusOutlined, FundOutlined, DollarOutlined, SearchOutlined, TagsOutlined } from '@ant-design/icons';
...
export default function FundPage() {
  ...
  const loadWatchlist = useCallback(async () => {
    try {
      ...
      const fundList = data.filter((d) => d.type === 'fund');
      setWatchlist(fundList);
      return data;
    } catch {
  ...
  const loadInvestments = useCallback(async () => {
    try {
      ...
      setInvestments(data.filter((d) => d.type === 'fund'));
    } catch {
  ...
  useEffect(() => {
    const init = async () => {
      await Promise.all([loadWatchlist(), loadInvestments(), loadTags()]);
      setLoading(false);
    };
    init();
  ...
  // 获取基金的投资金额
  const getInvestmentAmount = (symbol: string) => {
  ...
  const handleSearch = async (keyword: string) => {
    ...
    try {
      ...
      setSearchResults(Array.isArray(results) ? results.filter((r) => r.type === 'fund') : []);
    } catch {
      message.error('搜索失败');
    } finally {
      setSearching(false);
  ...
  const handleAddFund = async (item: SearchResult) => {
    try {
      console.log('[FundPage] Adding fund:', item.symbol, item.name, 'fund');
      const res = await watchlistApi.add(item.symbol, item.name, 'fund');
      console.log('[FundPage] Add success:', res);
      ...
      console.error('[FundPage] Add failed:', error);
      message.error(error.response?.data?.error || '添加失败');
    }
  };

  const handleRemoveFund = async (symbol: string) => {
    try {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Space>
          <Title level={4} style={{ margin: 0 }}>
            <FundOutlined /> 自选基金 ({watchlist.length})
          </Title>
      ...
      {filteredWatchlist.length === 0 ? (
        <Card style={{ textAlign: 'center', padding: 40 }}>
          ...
          <Title level={5} type="secondary">暂无自选基金</Title>
          <Text type="secondary">点击上方"添加基金"按钮搜索添加</Text>
        </Card>
      ) : (
        <div
          style={{
          ...
          {filteredWatchlist.map((item) => {
            ...
            return (
              <StockCard
                ...
                onRemove={() => handleRemoveFund(item.symbol)}
                onChart={() => {}}
                onFundDetail={() => {
                  setFundSymbol(item.symbol);
      ...
      >
        <Space.Compact style={{ width: '100%', marginBottom: 16 }}>
          <Input
            placeholder="输入基金代码/名称，如: 广发、110011"
            value={searchKeyword}
      ...
      {/* 基金重仓归因弹窗 */}
      <Modal
        title={`基金重仓归因 - ${fundName} (${fundSymbol?.toUpperCase()})`}
        open={!!fundSymbol}
        onCancel={() => setFundSymbol(null)}
        footer={null}
        width={800}
        destroyOnClose
      >
        {fundSymbol && <FundDetail symbol={fundSymbol} name={fundName} />}
      </Modal>
