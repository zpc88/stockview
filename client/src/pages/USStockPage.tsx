...
import { PlusOutlined, DollarOutlined, SearchOutlined, TagsOutlined } from '@ant-design/icons';
...
export default function USStockPage() {
  ...
  const loadWatchlist = useCallback(async () => {
    try {
      ...
      const usList = data.filter((d) => d.type === 'us_stock');
      setWatchlist(usList);
      return data;
    } catch {
      message.error('加载自选美股失败');
      return [];
    }
  }, []);
  ...
  const loadInvestments = useCallback(async () => {
    try {
      ...
      setInvestments(data.filter((d) => d.type === 'us_stock'));
    } catch {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const usSymbols = data.filter((d) => d.type === 'us_stock').map((d) => d.symbol);
      if (usSymbols.length > 0) {
        socketService.onQuotes('usStockPage', (newQuotes) => {
          setQuotes((prev) => {
        ...
        socketService.subscribe(usSymbols);
      }
    };
    ...
    return () => {
      socketService.offQuotes('usStockPage');
  ...
  const handleSearch = async (keyword: string) => {
    ...
    try {
      ...
      setSearchResults(Array.isArray(results) ? results.filter((r) => r.type === 'us_stock') : []);
    } catch {
      message.error('搜索失败');
    } finally {
      setSearching(false);
  ...
  const handleAddStock = async (item: SearchResult) => {
    try {
      await watchlistApi.add(item.symbol, item.name, 'us_stock');
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Space>
          <Title level={4} style={{ margin: 0 }}>
            <DollarOutlined /> 自选美股 ({watchlist.length})
          </Title>
      ...
      {filteredWatchlist.length === 0 ? (
        <Card style={{ textAlign: 'center', padding: 40 }}>
          ...
          <Title level={5} type="secondary">暂无自选美股</Title>
          <Text type="secondary">点击上方"添加美股"按钮搜索添加</Text>
        </Card>
      ) : (
        <div
          style={{
      ...
      >
        <Space.Compact style={{ width: '100%', marginBottom: 16 }}>
          <Input
            placeholder="输入美股代码/名称，如: AAPL、苹果"
            value={searchKeyword}
                ...
                title={
                  <span>
                    ...
                    <Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
                      {item.symbol.toUpperCase().replace('US_', '')}
                    </Text>
                  </span>
                }
              />
      ...
      {/* K线图弹窗 */}
      <Modal
        title={`${chartName} (${chartSymbol?.toUpperCase().replace('US_', '')})`}
        open={!!chartSymbol}
